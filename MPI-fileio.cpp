...
/* THIS FILE IS DESIGNED FOR USE ON WPI'S TURING CLUSTER. It uses OpenMPI to accelerate certain operations. This version
will not work on systems without that set up properly, and maybe be somewhat more brittle with respect to invalid or
unusual user parameter values. This file has been uploaded so that it does not get lost in case something happens, but it
is not especially intended for public use.*/
...
void printFileVec(MPI_File *fds) {
    uint32_t printbuf[SINGLE_FILE_ENTRIES];
    MPI_Status status;
    for (int i = 0; i < TOTAL_VEC_SIZE / SINGLE_FILE_SIZE; i++) {
        MPI_File_read(fds[i], printbuf, SINGLE_FILE_ENTRIES, MPI_UNSIGNED, &status);
        for (int j = 0; j < SINGLE_FILE_ENTRIES; j++) {
            cout << double(printbuf[j]) / FIXED_POINT_SCALE << " ";
        }
    }
    cout << "\n";
}
 ...
 *  - Failing if the file already exists as a safety measure (x), //TODO: put back
 * and return its file descriptor.*/
MPI_File openFile(std::string filepath) {
    MPI_File fh;
    if (MPI_File_open(MPI_COMM_WORLD, filepath.c_str(), MPI_MODE_RDWR | MPI_MODE_CREATE, MPI_INFO_NULL, &fh) < 0) {
        perror("Erorr opening file for writing");
        exit(EXIT_FAILURE);
    }
    return fh;
    // TODO: specify size of chunks in contiguous access request
}

/* Create all the files for one vector, and write their file descriptors (fd's) to vecfds*/
void initVectorFDs(MPI_File *vecfds, std::string fileprefix, const bool zeroInit) {
    ...
    for (int i = 0; i < num_maps; i++) {
        MPI_File fd = openFile((fileprefix + "-" + std::to_string(i) + ".bin").c_str());
        vecfds[i] = fd;
        cout << "SNARFABILITY5" << endl;
        ...
        if (zeroInit) {
            ...
            //     }
            // };
            auto zerofile = [&zeros](MPI_File fd, uint64_t num_bytes, uint64_t offset) -> void {
                MPI_Status status;
                if (MPI_File_write_at(fd, offset, zeros, num_bytes, MPI_UNSIGNED_CHAR, &status) < 0) {
                    perror("Error zeroing out the files");
                    exit(EXIT_FAILURE);
                }
            };
            cout << "SNARFABILITY" << endl;
            // TODO: Careful! Don't know whether the offset is in bytes or data type size because it doesnt say anywhere
            std::thread threads[NUM_THREADS - 1];
            uint64_t prev_total = 0;
            for (int j = 0; j < NUM_THREADS - 1; j++) {
                uint64_t total = ceil((j + 1) * double(SINGLE_FILE_SIZE) / NUM_THREADS);
                threads[j] = std::thread(zerofile, vecfds[i], total - prev_total, prev_total);
                prev_total = total;
            }
            zerofile(vecfds[i], SINGLE_FILE_SIZE - prev_total, prev_total);
            for (int j = 0; j < NUM_THREADS - 1; j++) {
                threads[j].join();
            }
            cout << "SNARFABILITY2" << endl;
        }
...
void copy_mem(uint32_t *memmap, MPI_File *vecfds, uint64_t num_elements, uint64_t read_offset, bool file_to_mem) {
    auto startTime = std::chrono::system_clock::now();

    // Lambda function for parallel copying to/from external memory
    auto parallel_copy = [&file_to_mem](uint32_t *memmap, MPI_File filemap, const uint64_t intra_file_offset,
                                        const uint64_t num_els) -> void {
        std::thread threads[NUM_THREADS];
        uint64_t prev_total = 0;
        MPI_Status status;  // not actually used
        for (int i = 0; i < NUM_THREADS; i++) {
            uint64_t total = ceil((i + 1) * num_els / double(NUM_THREADS));
            if (file_to_mem) {
                // TODO:RESTORE THREADING HERE
                //  threads[i] = std::thread(pread, filemap, memmap + prev_total, (total - prev_total) *
                //  sizeof(uint32_t),
                //                           intra_file_offset * sizeof(uint32_t) + prev_total * sizeof(uint32_t));
                threads[i] = std::thread(MPI_File_read_at, filemap, intra_file_offset + prev_total, memmap + prev_total,
                                         total - prev_total, MPI_UNSIGNED, &status);

                // MPI_File_read_at(filemap, intra_file_offset * sizeof(uint32_t), memmap, num_els, MPI_UNSIGNED,
                // &status);

            } else {
                // threads[i] = std::thread(pwrite, filemap, memmap + prev_total, (total - prev_total) *
                // sizeof(uint32_t),
                //                          intra_file_offset * sizeof(uint32_t) + prev_total * sizeof(uint32_t));
                threads[i] = std::thread(MPI_File_write_at, filemap, intra_file_offset + prev_total,
                                         memmap + prev_total, total - prev_total, MPI_UNSIGNED, &status);

                // MPI_File_write_at(filemap, intra_file_offset * sizeof(uint32_t), memmap, num_els, MPI_UNSIGNED,
                //                   &status);
                // TODO: try iwrite_at
            }
            prev_total = total;
        }
        // MPI_File_write_at(filemap, intra_file_offset * sizeof(uint32_t) + prev_total * sizeof(uint32_t),
        //                   memmap + prev_total, num_els - prev_total, MPI_UNSIGNED, &status);
        // TODO: test if actually makes any difference
...
uint32_t subtract_and_find_min_parallel(uint32_t *readmap, MPI_File *fdsv1, MPI_File *fdsv2) {
    ...
    for (uint64_t i = 0; i < (2 * TOTAL_VEC_SIZE / LOOP_CHUNK_SIZE); i++) {
        if (i % NUM_MACHINES != rank) continue;  // each physical machine calculates distinct parts
    ...
    // Find mininum across all machines, only root (rank=0) gets result
    uint32_t global_min;
    MPI_Reduce(&min_change, &global_min, 1, MPI_UNSIGNED, MPI_MIN, 0, MPI_COMM_WORLD);
    return global_min;
}
...
void recursive_driver(MPI_File *fdsv1, MPI_File *fdsv2, uint32_t *readmap, uint32_t *writemap) {
    ...
    int iter = -1;
    int tot_iter = 0;  // TODO: REMOVE tot_iter
    // This lambda performs the actual recursion
    std::function<void(uint64_t[2], uint64_t, int, int)> recurse_Lcombined;
    recurse_Lcombined = [&fdsv1, &fdsv2, &readmap, &writemap, &recurse_Lcombined, &inv_access_ranges_of, &iter,
                         &tot_iter](uint64_t offsets[2], uint64_t index_offset, int depth, int stage) -> void {
        ...
        if (depth < RECURSE_STOP_DEPTH) {
            ...
            switch (stage) {
                ...
                case 2: {
                  ...
                  // Check if this machine should execute this iteration, and skip it if not
            tot_iter += 1;

            iter = (iter + 1) % NUM_MACHINES;
            if (iter != rank) {
                return;
            }
            ...
            // cout << "TOT ITER " << tot_iter << " (start_i, end_i) " << start_i << " " << end_i << "\n";
            ...
            // for (int j = 0; j < LOOP_CHUNK_SIZE / (4); j++) {
            //     cout << double(readmap[j]) / FIXED_POINT_SCALE << " ";
            // }
            // cout << "\n";
...
void FeasibleTriplet(int n) {
    ...
    MPI_File v1fds[TOTAL_VEC_SIZE / SINGLE_FILE_SIZE];  // contains file descriptors of the files that comprise v1
    MPI_File v2fds[TOTAL_VEC_SIZE / SINGLE_FILE_SIZE];  // contains file descriptors of the files that comprise v2

    if (rank == 0) {
        initVectorFDs(v1fds, "./filev1", true);  // TODO: utilize all machines to zero init the files
    ...
    if (rank != 0) {
        initVectorFDs(v1fds, "./filev1", false);
        initVectorFDs(v2fds, "./filev2", false);
    }
    ...
    for (int i = 2; i < n + 1; i++) {
        ...
        cout << "Elapsed time of proc " << rank << " for F (s): " << secondsSince(start2) << endl;
        fflush(NULL);  // flush ALL file buffers
        for (int j = 0; j < TOTAL_VEC_SIZE / SINGLE_FILE_SIZE; j++) {
            MPI_File_sync(v2fds[j]);  // TODO: check if needed
        }
...
void start_MPI() {
    ...
    assert((void("Number of processes must equal NUM_MACHINES"), world_size == NUM_MACHINES));
    // Get rank (process 'ID', ranges from 0...num of processes -1)
    ...
    printf("Processor %s has rank %d out of %d processors\n", processor_name, rank, world_size);
    std::flush(cout);
    MPI_Barrier(MPI_COMM_WORLD);
}

int main() {
    assert(SINGLE_FILE_SIZE <= TOTAL_VEC_SIZE);
    start_MPI();
    if (rank == 0) {
